...
class UserKeyUpdateRequest(BaseModel):
    ...
    key: str | None = None
    image_quota_limit: int | None = None
    image_quota_used: int | None = None
...
def create_router() -> APIRouter:
    ...
    async def create_user_key(body: UserKeyCreateRequest, authorization: str | None = Header(default=None)):
        ...
        try:
            item, raw_key = auth_service.create_key(
                role="user",
                name=body.name,
                image_quota_limit=body.image_quota_limit,
            )
    ...
    async def update_user_key(
        ...
        if "key" in fields_set and body.key is not None:
            updates["key"] = body.key
        if "image_quota_limit" in fields_set:
            updates["image_quota_limit"] = body.image_quota_limit
        if "image_quota_used" in fields_set and body.image_quota_used is not None:
            updates["image_quota_used"] = body.image_quota_used
        if not updates:
    ...
    async def get_account_quota_summary(authorization: str | None = Header(default=None)):
        ...
        summary = account_service.get_image_quota_summary()
        image_quota_limit = identity.get("image_quota_limit")
        image_quota_used = identity.get("image_quota_used")
        image_quota_remaining = identity.get("image_quota_remaining")
        is_user = identity.get("role") == "user"
        return {
            **summary,
            "scope": "user" if is_user else "pool",
            "display_quota": image_quota_remaining if is_user else summary.get("available_quota", 0),
            "image_quota_limit": image_quota_limit,
            "image_quota_used": image_quota_used,
            "image_quota_remaining": image_quota_remaining,
        }
