...
class AuthService:
    ...
    def _normalize_item(self, raw: object) -> dict[str, object] | None:
        ...
        image_quota_limit = self._optional_non_negative_int(raw.get("image_quota_limit"))
        image_quota_used = self._non_negative_int(raw.get("image_quota_used"))
        return {
            "id": item_id,
            "name": name,
            "role": role,
    ...
    def _public_item(item: dict[str, object]) -> dict[str, object]:
        image_quota_limit = AuthService._optional_non_negative_int(item.get("image_quota_limit"))
        image_quota_used = AuthService._non_negative_int(item.get("image_quota_used"))
        return {
            "id": item.get("id"),
            "name": item.get("name"),
            "role": item.get("role"),
    ...
    def create_key(
        ...
        *,
        role: AuthRole,
        name: str = "",
        image_quota_limit: int | None = None,
        ...
        with self._lock:
            ...
            item = {
                ...
                "created_at": _now_iso(),
                "last_used_at": None,
                "image_quota_limit": self._optional_non_negative_int(image_quota_limit),
                "image_quota_used": 0,
            }
        ...
        *,
        ...
        with self._lock:
            ...
            for index, item in enumerate(self._items):
                ...
                if "image_quota_limit" in updates:
                    next_item["image_quota_limit"] = self._optional_non_negative_int(updates.get("image_quota_limit"))
                if "image_quota_used" in updates:
                    next_item["image_quota_used"] = self._non_negative_int(updates.get("image_quota_used"))
                self._items[index] = next_item
                self._save()
                return self._public_item(next_item)
        return None

    def consume_image_quota(self, identity: dict[str, object], count: int = 1) -> dict[str, object] | None:
        ...
        with self._lock:
            ...
            for index, item in enumerate(self._items):
                if item.get("id") != key_id or item.get("role") != "user":
                    continue
                limit = self._optional_non_negative_int(item.get("image_quota_limit"))
                used = self._non_negative_int(item.get("image_quota_used"))
                if limit is not None and used + amount > limit:
                    raise ValueError("生图次数已用完")
                next_item = dict(item)
                next_item["image_quota_used"] = used + amount
                self._items[index] = next_item
                self._save()
                return self._public_item(next_item)
        raise ValueError("user key not found")
