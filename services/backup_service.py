...
class S3StorageClient:
    """通用 S3 兼容存储客户端，支持 Cloudflare R2、Backblaze B2、AWS S3、MinIO 等"""

    # 各 provider 的默认 endpoint 和 region
    _PRESETS = {
        "cloudflare_r2": {
    ...
    def __init__(self, settings: dict[str, object]) -> None:
        self.provider = _clean(settings.get("provider")) or "cloudflare_r2"
        ...
        self.custom_endpoint = _clean(settings.get("s3_endpoint"))
        self.custom_region = _clean(settings.get("s3_region"))
    ...
    def endpoint(self) -> str:
        ...
        template = self._preset.get("endpoint_template", "")
        if template and self.account_id:
            return template.format(account_id=self.account_id, region=self.region)
        return ""
    ...
    def host(self) -> str:
        ...
        ep = self.endpoint
        if not ep:
            raise BackupError(f"无法确定存储端点，请检查 provider 和 s3_endpoint 配置")
        return str(urlparse(ep).hostname or "")

    def validate(self) -> None:
        missing = []
        if not self.account_id and self.provider == "cloudflare_r2":
        ...
        if missing:
            raise BackupError(f"存储配置不完整：缺少 {'、'.join(missing)}")
    ...
    def _request(
        ...
        *,
        ...
        url = f"{self._endpoint_url}{object_path}"
        if encoded_query:
            url += f"?{encoded_query}"
...
class BackupService:
    ...
    def test_connection(self) -> dict[str, object]:
        client = S3StorageClient(config.get_backup_settings())
        try:
    ...
    def _apply_rotation(self, client: S3StorageClient, keep: int) -> None:
        if keep <= 0:
            return
