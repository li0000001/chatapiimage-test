...
def _normalize_backup_settings(value: object) -> dict[str, object]:
    source = value if isinstance(value, dict) else {}
    provider = str(source.get("provider") or os.getenv("BACKUP_PROVIDER") or "").strip().lower()
    if provider not in ("cloudflare_r2", "backblaze_b2", "aws_s3", "generic_s3"):
        provider = "cloudflare_r2"
    return {
        "enabled": _normalize_bool(source.get("enabled") or os.getenv("BACKUP_ENABLED"), False),
        "provider": provider,
        "s3_endpoint": str(source.get("s3_endpoint") or os.getenv("BACKUP_S3_ENDPOINT") or "").strip(),
        "s3_region": str(source.get("s3_region") or os.getenv("BACKUP_S3_REGION") or "").strip(),
        "account_id": str(source.get("account_id") or os.getenv("BACKUP_ACCOUNT_ID") or "").strip(),
        "access_key_id": str(source.get("access_key_id") or os.getenv("BACKUP_ACCESS_KEY_ID") or "").strip(),
        "secret_access_key": str(source.get("secret_access_key") or os.getenv("BACKUP_SECRET_ACCESS_KEY") or "").strip(),
        "bucket": str(source.get("bucket") or os.getenv("BACKUP_BUCKET") or "").strip(),
        "prefix": str(source.get("prefix") or os.getenv("BACKUP_PREFIX") or "backups").strip().strip("/") or "backups",
        "interval_minutes": _normalize_positive_int(source.get("interval_minutes") or os.getenv("BACKUP_INTERVAL_MINUTES"), 360, 1),
        "rotation_keep": _normalize_positive_int(source.get("rotation_keep") or os.getenv("BACKUP_ROTATION_KEEP"), 10, 0),
        "encrypt": _normalize_bool(source.get("encrypt") or os.getenv("BACKUP_ENCRYPT"), False),
        "passphrase": str(source.get("passphrase") or os.getenv("BACKUP_PASSPHRASE") or "").strip(),
