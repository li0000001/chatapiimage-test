...
function ImageManagerContent() {
  ...
  const lightboxImages = filteredItems.map((item) => ({
    id: item.name,
    src: normalizeBrowserImageUrl(item.url),
        ...
        <DialogContent className="max-w-sm overflow-hidden rounded-2xl">
          ...
          {deleteTarget ? (
            <div className="flex items-center gap-3 overflow-hidden rounded-xl border border-stone-200 bg-stone-50 p-3">
              <img
                src={normalizeBrowserImageUrl(deleteTarget.thumbnail_url || deleteTarget.url)}
                alt=""
                className="size-16 shrink-0 rounded-lg object-cover"
                onError={(e) => {
                  const fallbackSrc = normalizeBrowserImageUrl(deleteTarget.url);
