...
import { normalizeBrowserImageUrl } from "@/lib/image-url";
...
function getStoredImageSrc(image: StoredImage) {
  ...
  return normalizeBrowserImageUrl(image.url || "");
}
...
async function downloadStoredImage(image: StoredImage, index: number) {
  ...
  if (image.b64_json) {
    ...
    const res = await fetch(normalizeBrowserImageUrl(image.url));
    blob = await res.blob();
  } else {
    return;
  }
...
export function ImageResults({
    ...
    return (
      <div className="flex h-full min-h-[260px] items-center justify-center text-center sm:min-h-[420px]">
        <div
          className={cn(
            "w-full max-w-4xl",
            isUserMode && "max-w-3xl rounded-[28px] border border-slate-200/70 bg-white/72 px-6 py-10 shadow-[0_20px_70px_-54px_rgba(15,23,42,0.5)] sm:px-10",
          )}
        >
          {isUserMode ? (
            <div className="mx-auto mb-5 flex size-13 items-center justify-center rounded-2xl bg-slate-950 text-white">
              <Sparkles className="size-5" />
            </div>
          ) : null}
          <h1
            className={cn(
              "text-2xl font-semibold tracking-tight text-stone-950 sm:text-3xl md:text-5xl",
              isUserMode && "text-slate-950 md:text-4xl",
            )}
            style={
              isUserMode
                ? undefined
          ...
          >
            {isUserMode ? "今天想生成什么画面？" : "Turn ideas into images"}
          </h1>
          <p
            className={cn(
              "mx-auto mt-3 max-w-[280px] text-sm italic tracking-[0.01em] text-stone-500 sm:mt-4 sm:max-w-none sm:text-[15px]",
              isUserMode && "max-w-[520px] text-slate-500 not-italic sm:text-sm",
            )}
            style={
              isUserMode
                ? undefined
          ...
          >
            {isUserMode
              ? "输入提示词开始生成，也可以上传或粘贴参考图继续编辑。每次结果都会自动进入本地历史。"
              : "在同一窗口里保留本地历史与任务状态，并从已有结果图继续发起新的无状态编辑。"}
          </p>
          {isUserMode ? (
            <div className="mt-7 grid gap-2 text-left text-sm text-slate-600 sm:grid-cols-3">
              <div className="rounded-2xl bg-slate-50 px-4 py-3">描述画面风格</div>
              <div className="rounded-2xl bg-teal-50 px-4 py-3 text-teal-800">上传参考图</div>
              <div className="rounded-2xl bg-amber-50 px-4 py-3 text-amber-800">继续编辑结果</div>
            </div>
          ) : null}
        </div>
      </div>
  ...
  return (
    <div className={cn("mx-auto flex w-full max-w-[980px] flex-col gap-5 sm:gap-8", isUserMode && "max-w-[1040px]")}>
