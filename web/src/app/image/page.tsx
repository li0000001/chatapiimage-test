...
async function fetchImageAsFile(url: string, fileName: string) {
  const response = await fetch(normalizeBrowserImageUrl(url));
  if (!response.ok) {
...
function taskDataToStoredImage(image: StoredImage, task: ImageTask): StoredImage {
  if (task.status === "success") {
    ...
    return {
      ...
      url: first.url ? normalizeBrowserImageUrl(first.url) : first.url,
...
function ImagePageContent({ isUserMode }: { isUserMode: boolean }) {
  ...
  const loadQuota = useCallback(async () => {
    try {
      ...
      if (data.scope === "user") {
        setAvailableQuota(data.display_quota === null || data.display_quota === undefined ? "不限" : String(Math.max(0, data.display_quota)));
        return;
      }
      setAvailableQuota(String(Math.max(0, data.display_quota ?? data.available_quota)));
    } catch {
    ...
    async (conversationId: string, image: StoredImage | StoredReferenceImage) => {
      try {
        const normalizedImage =
          "dataUrl" in image || !image.url
            ? image
            : {
                ...image,
                url: normalizeBrowserImageUrl(image.url),
              };
        const nextReference =
          "dataUrl" in normalizedImage
            ? {
                referenceImage: normalizedImage,
                file: dataUrlToFile(normalizedImage.dataUrl, normalizedImage.name, normalizedImage.type),
              }
  ...
  return (
    <>
      ...
      >
        <div
          className={cn(
            "hidden h-full min-h-0 border-r border-stone-200/70 pr-3 lg:block",
            isUserMode && "rounded-[22px] border border-slate-200/70 bg-slate-50/72 p-3",
          )}
        >
          <ImageSidebar
        ...
        <div className="flex min-h-0 flex-col gap-2 overflow-hidden sm:gap-4">
          <div className="flex shrink-0 items-center justify-between gap-2 px-1 lg:hidden">
            <Button
              variant="outline"
              className={cn(
                "h-10 flex-1 rounded-2xl border-stone-200 bg-white/90 text-stone-700 shadow-sm",
                isUserMode && "border-slate-200 bg-white text-slate-700",
              )}
            ...
            <Button
              className={cn("h-10 rounded-2xl bg-stone-950 text-white shadow-sm", isUserMode && "bg-slate-950")}
            ...
            <Button
              variant="outline"
              className={cn(
                "h-10 rounded-2xl border-stone-200 bg-white/85 px-3 text-stone-600 shadow-sm",
                isUserMode && "border-slate-200 bg-white text-slate-500",
              )}
...
export default function ImagePage() {
  ...
  return <ImagePageContent isUserMode={session.role === "user"} />;
}
