...
const PROVIDER_OPTIONS = [
  ...
  { value: "generic_s3", label: "通用 S3 兼容存储" },
] as const;
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold tracking-tight">S3 备份管理</h2>
                <p className="text-sm text-stone-500">将关键数据定时备份到 S3 兼容存储桶（Cloudflare R2、Backblaze B2、AWS S3 等），支持可选加密、轮替、手动执行与历史清理。</p>
              </div>
            </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">存储提供商</label>
            <Select value={String(backup.provider || "cloudflare_r2")} onValueChange={(value) => setBackupField("provider", value)}>
          ...
          {(backup.provider === "generic_s3" || backup.provider === "backblaze_b2" || backup.provider === "aws_s3") && (
            <div className="space-y-2">
              <label className="text-sm text-stone-700">S3 Region（可选）</label>
              <Input value={String(backup.s3_region || "")} onChange={(event) => setBackupField("s3_region", event.target.value)} placeholder="us-west-004" className="h-10 rounded-xl border-stone-200 bg-white" />
              <p className="text-xs text-stone-500">留空则使用默认 Region。Backblaze B2 常用：us-west-004、us-east-005、eu-central-003</p>
            </div>
          )}

          <div className="space-y-2">
            <label className="text-sm text-stone-700">{backup.provider === "cloudflare_r2" ? "Cloudflare Account ID" : "Account ID / 应用密钥 ID"}</label>
            ...
            {backup.provider === "cloudflare_r2" ? null : <p className="text-xs text-stone-500">Cloudflare R2 填 Account ID；其他提供商可留空（通过 Endpoint 连接）</p>}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">存储桶内对象前缀，例如 `backups/prod`。</p>
          </div>
          <div className="space-y-2">
