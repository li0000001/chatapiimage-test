...
function formatImageQuota(item: UserKey) {
  if (item.image_quota_limit === null || item.image_quota_limit === undefined) {
    return `不限次数，已用 ${item.image_quota_used || 0}`;
  }
  return `剩余 ${item.image_quota_remaining ?? 0} / ${item.image_quota_limit}，已用 ${item.image_quota_used || 0}`;
}
...
export function UserKeysCard() {
  ...
  const [imageQuotaLimit, setImageQuotaLimit] = useState("");
  ...
  const [editImageQuotaLimit, setEditImageQuotaLimit] = useState("");
  const [editImageQuotaUsed, setEditImageQuotaUsed] = useState("");
  ...
  const handleCreate = async () => {
    setIsCreating(true);
    try {
      const data = await createUserKey(name.trim(), parseOptionalLimit(imageQuotaLimit));
      setItems(data.items);
  ...
  const openEditDialog = (item: UserKey) => {
    ...
    setEditKey("");
    setEditImageQuotaLimit(item.image_quota_limit === null || item.image_quota_limit === undefined ? "" : String(item.image_quota_limit));
    setEditImageQuotaUsed(String(item.image_quota_used || 0));
  };
  ...
  const handleEdit = async () => {
    ...
    const nextQuotaLimit = parseOptionalLimit(editImageQuotaLimit);
    const currentQuotaLimit = item.image_quota_limit === undefined ? null : item.image_quota_limit;
    const nextQuotaUsed = parseCounter(editImageQuotaUsed);
    const currentQuotaUsed = item.image_quota_used || 0;
    if (
      trimmedName === item.name &&
      !trimmedKey &&
      nextQuotaLimit === currentQuotaLimit &&
      nextQuotaUsed === currentQuotaUsed
    ) {
      setEditingItem(null);
      return;
    }
    ...
    try {
      const data = await updateUserKey(item.id, {
        ...
        ...(nextQuotaLimit !== currentQuotaLimit ? { image_quota_limit: nextQuotaLimit } : {}),
        ...(nextQuotaUsed !== currentQuotaUsed ? { image_quota_used: nextQuotaUsed } : {}),
      });
      setItems(data.items);
      setEditingItem(null);
      setEditKey("");
    ...
    <>
                  ...
                  <div key={item.id} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                        ...
                        <span>生图额度 {formatImageQuota(item)}</span>
                      </div>
                    </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="rounded-2xl p-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-stone-700">生图次数限制（可选）</label>
            <Input
              type="number"
              min={0}
              value={imageQuotaLimit}
              onChange={(event) => setImageQuotaLimit(event.target.value)}
        ...
        <DialogContent className="rounded-2xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-sm font-medium text-stone-700">生图次数限制</label>
                <Input
                  type="number"
                  min={0}
                  value={editImageQuotaLimit}
                  onChange={(event) => setEditImageQuotaLimit(event.target.value)}
                  placeholder="留空表示不限制"
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-stone-700">已用次数</label>
                <Input
                  type="number"
                  min={0}
                  value={editImageQuotaUsed}
                  onChange={(event) => setEditImageQuotaUsed(event.target.value)}
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              </div>
            </div>
