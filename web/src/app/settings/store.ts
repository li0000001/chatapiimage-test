...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    backup: {
      ...
      s3_endpoint: String(backup.s3_endpoint || ""),
      s3_region: String(backup.s3_region || ""),
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        backup: {
          ...
          s3_endpoint: String(config.backup?.s3_endpoint || "").trim(),
          s3_region: String(config.backup?.s3_region || "").trim(),
  ...
  testBackup: async () => {
    ...
    try {
      ...
      toast.success(`存储连接正常（HTTP ${data.result.status}）`);
    } catch (error) {
