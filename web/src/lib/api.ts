...
export type BackupSettings = {
  enabled: boolean;
  provider: "cloudflare_r2" | "backblaze_b2" | "aws_s3" | "generic_s3" | string;
  s3_endpoint: string;
  s3_region: string;
  account_id: string;
...
export async function fetchAccountQuotaSummary() {
  return httpRequest<AccountQuotaSummary>("/api/accounts/quota");
}
...
export async function createUserKey(name: string, imageQuotaLimit?: number | null) {
  return httpRequest<{ item: UserKey; key: string; items: UserKey[] }>("/api/auth/users", {
    method: "POST",
    body: { name, image_quota_limit: imageQuotaLimit ?? null },
  });
}
